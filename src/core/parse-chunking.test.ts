...
function roundTrip(result: ParseResult, chunkSize: number): ReturnType<ChunkAssembler['finish']> {
...
describe('parse-chunking parity', () => {
  it('reassembles an identical result across the chunk boundary', () => {
    const result = makeSyntheticResult(530, 3);
    const assembled = roundTrip(result, DEFAULT_SESSION_CHUNK_SIZE);
  ...
  it('preserves orphan edit and source entries via the done payload', () => {
    const result = makeSyntheticResult(10, 2);
    const assembled = roundTrip(result, 4);
  ...
  it('is invariant to chunk size', () => {
    ...
    const a = roundTrip(result, 1);
    const b = roundTrip(result, 1000);
  ...
  it('emits the expected number of chunks for the session count', () => {
    const result = makeSyntheticResult(250, 1);
    let chunks = 0;
    emitResultChunks(result, () => { chunks++; }, 100);
    expect(chunks).toBe(3); // 100 + 100 + 50
  });

  it('handles an empty result without emitting chunks', () => {
    ...
    let chunks = 0;
    const done = emitResultChunks(empty, () => { chunks++; });
